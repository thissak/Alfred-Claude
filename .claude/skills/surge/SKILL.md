...
Notion MCP로 결과를 저장한다.

1. **부모 페이지**: "Surge — 종목 이상패턴 분석" (page_id: `332b83e1-2dd9-81d1-842f-db9d5f645de8`)
2. 이 페이지 아래에 자식 페이지를 생성한다:
   - 제목: `{종목명} ({종목코드}) — {오늘 날짜}`
   - 아이콘: 📈 (급등 위주) 또는 📉 (급락 위주)
3. 내용 구성:
   - 분석 기간·최종 종가 요약
   - 핵심 패턴 테이블 (Notion table 포맷, 급등은 green_bg, 급락은 red_bg)
   - 인사이트 (번호 목록)
   - 리스크 (번호 목록)
   - 생성 정보 (날짜, 데이터 출처)
4. 같은 종목·같은 날짜 페이지가 이미 있으면 새로 만들지 않고 업데이트
