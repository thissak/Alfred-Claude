...
```
codex exec "Give me the latest Iran war update as of {today}. Cover: 1) Military situation 2) Ceasefire/diplomacy 3) Energy markets (Brent, Hormuz) 4) Economic indicators (VIX, Asian markets). Be factual, cite sources. Search the web." < /dev/null 2>&1 | head -80
```
...
리포트 출력 후 **항상 자동 저장**한다 (별도 인자 불필요):
- `docs/iran-war/daily/{today}.md`에 리포트 저장
- `docs/iran-war/analysis.md` 누적 업데이트 (핵심 변화·경제 지표·시나리오 테이블)
- 기존 파일이 있으면 덮어쓰기 (최신 데이터 우선)
