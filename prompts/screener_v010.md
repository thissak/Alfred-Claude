...
1. **반복 선별 감점** — 009540이 5/5 선별, 과적합의 최대 원인
2. **ETF 제외 철회 → 조건부 포함** — 069500이 3/5 유지했으나 0/5 선별, v009 blanket 제외가 체계적 누락 원인
...
ETF는 개별주와 동일한 Gate 기준을 적용한다. Gate 통과 ETF가 1건 이상이면 최소 1건은 평가에 포함한다.
...
## 반복 선별 감점
동일 종목이 직전 3라운드 연속 선별된 경우 conviction을 1단계 하향한다.
과적합 방지 목적이며, 하향 후에도 score 기준 선별 가능.
...
**v010 변경 2건:**
1. **ETF 제외 철회 → 조건부 포함**: v009의 blanket 제외를 철회. ETF도 개별주와 동일 Gate 적용, Gate 통과 ETF가 있으면 최소 1건 평가 의무. 069500이 3/5 정배열 유지했으나 0/5 선별된 체계적 누락 해소
2. **반복 선별 감점 신설**: 동일 종목 3연속 선별 시 conviction 1단계 자동 하향. 009540이 5/5 과적합된 문제 방지, 종목 다양성 확보